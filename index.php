...
if (isset($_POST['action'])) {
  switch($_POST['action']) {
    ...
    case 'toggle':
      ...
      $id = $_POST['id'];
      if(is_numeric($id)) {
        $updateQuery = 'UPDATE todo SET done = NOT done WHERE id = ' .intval($id); // Valentine Vecchi
    ...
    case 'delete':
      ...
      $id = $_POST['id'];
      if(is_numeric($id)) {
        $deleteQuery = 'DELETE FROM todo WHERE id = ' . intval($id); // IMPLEMENT ME
 ...
 */
$selectQuery = 'SELECT * FROM todo ORDER BY created_at DESC' ; // Valentine Vecchi
